...
gene2 <- read.table("./data/RNA-seq/20200824/DEG/sgNTC_vs_sgKAT7_DEG_clean.xls", header = T)
gene2_up <- gene2[gene2$cluster == "sgKAT7", ]
gene2_down <- gene2[gene2$cluster == "NTC", ]

tmp1 <- intersect(as.character(gene1_up$gene), as.character(gene2_down$gene))
write.table(tmp1, "LateUP_and_sgKAT7DOWN.xls", row.names = F, col.names = F, sep = "\t", quote = F)
tmp2 <- intersect(as.character(gene1_down$gene), as.character(gene2_up$gene))
write.table(tmp2, "LateDOWN_and_sgKAT7UP.xls", row.names = F, col.names = F, sep = "\t", quote = F)
...
# pdf("LateUP_and_sgKAT7DOWN.pdf", width = 5, height = 5)
pdf("LateDOWN_and_sgKAT7UP.pdf", width = 5, height = 5)
grid.draw(V)
dev.off()
