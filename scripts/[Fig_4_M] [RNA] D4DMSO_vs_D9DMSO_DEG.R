...
cts <- expression.count[, grepl("D4_DMSO_", colnames(expression.count)) | grepl("D9_DMSO_", colnames(expression.count))]
colnames(cts)
...
coldata$Day <- factor(coldata$Day, levels = c("D4","D9"))
...
mydata[mydata$logFC <= -0.585 & mydata$FDR <= 0.05, "cluster"] <- "D4"
mydata[mydata$logFC >= 0.585 & mydata$FDR <= 0.05, "cluster"] <- "D9"
mydata[is.na(mydata$cluster), "cluster"] <- "pother"
mydata <- mydata[order(mydata$cluster, decreasing = T), ]
table(mydata$cluster)

pdf("D4_vs_D9_DEG_volcan.pdf", width = 6, height = 5)
ggplot(mydata, aes(x=mydata$logFC, y=-log10(mydata$FDR))) +
  geom_point(alpha = 0.8, aes(color = cluster, size = cluster)) +
  xlab("log2 (Fold Change)") + ylab("-log10 (FDR)") +
  scale_color_manual(values = c("D4"="blue","D9"="red","pother"="#D9D9D9")) +
  scale_size_manual(values = c("D4"=1.5,"D9"=1.5,"pother"=1)) +
...
write.table(output, "D4_vs_D9_DEG.xls", row.names = F, col.names = T, sep = "\t", quote = F)
write.table(mydata, "D4_vs_D9_DEG_clean.xls", row.names = F, col.names = T, sep = "\t", quote = F)
...
pdf("D4_vs_D9_DEG_heatmap.pdf", width = 5, height = 5)
