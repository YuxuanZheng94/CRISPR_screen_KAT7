...
cts <- expression.count[, grepl("Liver_2m_", colnames(expression.count)) | grepl("Liver_28m_", colnames(expression.count))]
colnames(cts)
...
coldata$Day <- factor(coldata$Day, levels = c("2m","28m"))
...
mydata[mydata$logFC <= -0.585 & mydata$FDR <= 0.05, "cluster"] <- "2m"
mydata[mydata$logFC >= 0.585 & mydata$FDR <= 0.05, "cluster"] <- "28m"
mydata[is.na(mydata$cluster), "cluster"] <- "pother"
mydata <- mydata[order(mydata$cluster, decreasing = T), ]
table(mydata$cluster)

pdf("liver_2m_vs_28m_DEG_volcan.pdf", width = 6, height = 5)
ggplot(mydata, aes(x=mydata$logFC, y=-log10(mydata$FDR))) +
  geom_point(alpha = 0.8, aes(color = cluster, size = cluster)) +
  xlab("log2 (Fold Change)") + ylab("-log10 (FDR)") +
  scale_color_manual(values = c("2m"="blue","28m"="red","pother"="#D9D9D9")) +
  scale_size_manual(values = c("2m"=1.5,"28m"=1.5,"pother"=1)) +
...
write.table(output, "liver_2m_vs_28m_DEG.xls", row.names = F, col.names = T, sep = "\t", quote = F)
write.table(mydata, "liver_2m_vs_28m_DEG_clean.xls", row.names = F, col.names = T, sep = "\t", quote = F)
...
pdf("liver_2m_vs_28m_DEG_heatmap.pdf", width = 5, height = 5)
